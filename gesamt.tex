...
\newcolumntype{G}{>{\columncolor{mgray}}c}
...
% Nachfolgend 'Wörter' durch 'Einträge' ersetzt
...
% Ich würde 1., 2. usw. als Aufzählung verwenden, liest sich besser
...
\hline
\rowcolor{mgray}
\textsc{Stute}	&	\cellcolor{intersect}\color{white}$\bigtimes$	&&	\cellcolor{intersect}\color{white}$\bigtimes$	&	$\bigtimes$	&\\
\hline
\textsc{Fohlen}	&	$\bigtimes$	&&&&	$\bigtimes$\\
\hline
\rowcolor{mgray}
\textsc{Stutfohlen}	&	\cellcolor{intersect}\color{white}$\bigtimes$	&&	\cellcolor{intersect}\color{white}$\bigtimes$	&&	$\bigtimes$\\
\hline
...
Das Ordnungsdiagramm in Abbildung~\ref{fig:hasse}, Seite~\pageref{fig:hasse} ist eine vollständige Darstellung der Kreuztabelle unter\emph{~\nameref{subsec:fba}} auf Seite~\pageref{subsec:fba}.
...
Man könnte zwar das Merkmal \emph{\unit[17]{mmHg}} zuweisen, dies würde jedoch allenfalls Objekte, die mit dem Merkmal \emph{\unit[17]{mmHg}} versehen wurden, untereinander vergleichbar machen, da es die Formale Begriffsanalyse nicht vorsieht, Beziehungen zwischen Merkmalen zu kennzeichnen oder Merkmale als Zahlenwerte zu interpretieren. 
Ein Objekt mit dem Merkmal \emph{\unit[16]{mmHg}} wäre somit genauso wenig bedeutungsgleich zu einem Objekt mit dem Merkmal \emph{\unit[17]{mmHg}} wie eines mit dem Merkmal \emph{\unit[6]{mmHg}}, es sei denn die Bedeutungsähnlichkeit würde durch weitere Merkmale deutlich gemacht werden. 
...
Hier ist der Aufbau des Merkmalsraums schwierig, da die Domäne komplex ist und nicht von vorneherein feststeht, welche Objekte geordnet werden sollen.
...
Der Umstand, dass lediglich geprüft werden muss, ob ein Merkmal einem untersuchten Objekt zugewiesen werden kann oder nicht, bedarf nur sehr einfacher Datenstrukturen.
...
Im Kapitel~\ref{sec:visualization_0}\emph{~\nameref{sec:visualization_0}} stellen wir vor, wie wir Nutzern des Wörterbuchs die Möglichkeit geben, sich einzelne Begriffsverbände visualisieren zu lassen und interaktiv zu steuern.
...
Für einen kompletten formalen Kontext, wie ihn die Formale Begriffsanalyse beschreibt, sind eine Menge an Objekten und eine Menge an Merkmalen aber nicht ausreichend: der Bezug der Elemente der einen Menge zu den Elementen der anderen Menge fehlt. 
...
Ein Merkmal bekommt ein Objekt entweder zugewiesen oder nicht. 
Diese Beziehung zwischen der Objekt- und der Merkmalsmenge bildet die Grundlage der Analyse. 
...
Anhand dieser sollten geeignete Wortmuster mit semantischem Inhalt ausgewählt und dann in einem weiteren Durchlauf der Wortmusteranalyse berücksichtigt werden. 
...
Anschließend wurde die Liste ausgehend vom am häufigsten vorkommenden n-Gramm bis zu einem minimalen Vorkommen von 6 manuell abgearbeitet und jedes n-Gramm jeweils nach semantischem Inhalt sowie semantischer Eindeutigkeit überprüft.
...
% disease durch illness ersetzt (damit einheitlich, siehe /dys/ für 'illness' oben)
...
Aufgrund der großen Menge an bereitgestellten Informationen haben wir uns dazu entschieden, die Online-Enzyklopädie Wikipedia als Wissensquelle für das \enquote{Wörterbuch der Augenheilkunde} zu berücksichtigen. 
Auch wenn anfänglich sowohl die englische, als auch die deutsche Version der Wikipedia für die Informationsextraktion herangezogen wurden, entschied man sich schließlich für die englische Wikipedia als Wissensquelle, um einerseits den Merkmalsraum einsprachig zu halten und andererseits, weil die englische Wikipedia deutlich mehr Informationen aus dem Bereich der Augenheilkunde enthält.
Außerdem verwendet die englische Sprache weniger Flexionen als die deutsche, was den linguistischen Abgleich von Informationen der Wikipedia mit den Einträgen aus dem Wörterbuch vereinfacht. 
...
% kleiner sem. Fehler: eingeordnet nun durch zugeordnet ersetzt
...
Der Aufwand damit eine fehlerfreie Menge an Abstracts zu extrahieren zeigte sich aber als zu groß, weil die Abstracts keine vollkommen einheitliche Struktur hatten, und die XSL-Templates immer wieder angepasst werden mussten um die Treffermenge zu erhöhen.
Wenngleich XSLT für drei Funktionen reguläre Ausdrücke zulässt und die Syntax und der Umfang im Wesentlichen den \emph{Perl Compatible Regular Expressions} entspricht, ist die technische Dokumentation des W3C schwierig zu interpretieren gewesen und die Dokumentation von \emph{xsltproc} nicht sehr hilfreich beim Aufspüren von Fehlern gewesen.\footnote{Beispielsweise extrahierte \emph{xsltproc} aus testweise übergebenen Zeilen nicht, obwohl die Syntax der technischen Dokumentation entsprach. Die Dokumentation von \emph{xsltproc} enthält aber kaum Genaueres zur Implementation.} 
Meist wurde sich damit beholfen, die Templates etwas allgemeiner zu halten und die fraglichen Elemente über die Textwerkzeuge der Unix-Shell (\emph{grep, sed, awk}) und \emph{Perl} zu extrahieren.
Weil damit der wesentliche Vorteil der Verwendung von \emph{xsltproc} entfallen war, wurden in späteren Versuchen die gewünschten Elemente vollständig über diesen Weg extrahiert.
...
Ein typisches Beispiel dafür ist die Kategorie \enquote{Articles needing attention}, die selbst wiederum mehrere hundert Unterkategorien enthält (unter anderem zu jener Zeit sich selbst: \enquote{Medical Articles needing attention}). 
...
Es gibt nur ca. 500 Merkmale, die häufiger als dreimal zugewiesen wurden. 
...
% Hinzugefügt, dass es bei der Stoppwortfilterung um N-Gramme geht
...
% Begriffsverband durch Kontext ersetzt
...
Da es mit einem Part-of-Speech-Tagger vergleichsweise einfach ist, die entsprechenden Teile zu extrahieren, sollte der Versuch unternommen werden dem Merkmalscharakter möglichst nahe zu kommen, indem die Wörter, die unmittelbar verwendet werden um den in Frage stehenden Begriff zu beschreiben, als Merkmale zu genutzt werden. 
...
Um die Server zu schonen und die Zugriffe nicht so auffällig zu machen, wurden die Zugriffe unter anderem auf einen Zufallswert zwischen einer und 60 Sekunden eingestellt, die Download-Rate auf 15 Kb/s gedrosselt und die Anzahl der Verbindungsversuche je Artikel auf fünf Versuche beschränkt. 
...
% Wieder Begriffsverband durch Kontext ersetzt (in beiden folgenden Sätzen)
...
% Diesmal Kontext durch Begriffsverband ersetzt
...
Unser Script zur Visualisierung eines formalen Kontextes erzeugt mithilfe des Programms \emph{GraphViz} und der Skriptsprache PHP ein Ordnungsdiagramm ausgehend von einem Synset, das der Nutzer explizit ausgewählt hat. 
...
Auf Grundlage der Merkmale, die auf das vom Nutzer gewählte Synset zutreffen werden alle Synsets ausgewählt, auf die mindestens eines dieser Merkmale zutrifft. 
Mit dieser Synset- und Merkmalsmenge wird der Begriffsverband aufgebaut, dessen Ordnungsdiagramm dann direkt im Skript visualisiert wird. 
Der Vorteil dieses Vorgehens ist, dass ausgehend von einem Synset dessen Ober- und Unterbegriffe erkundet werden können. 

%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
%% Nachfolgender Satz neu, bitte prüfen
%% Die Menge der dem gewählten Synset zugewiesenen Merkmale bildet den kleinsten Begriff.
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
Die Menge der dem gewählten Synset zugewiesenen Merkmale bildet den kleinsten Begriff. 
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
%% Nachfolgender Satz neu, bitte prüfen
%% Die zugehörige Potenzmenge dieser Menge bildet, wie im Kapitel~\nameref{sec:hasse} beschrieben, alle Knoten des erzeigten Diagramms, einschließlich des größten Begriffs, der hier der leeren Menge entspricht.\footnote{Dies ist nicht immer zwingend der Fall. Der größte Begriff kann ein Merkmal umfassen, das auf alle Gegenstände zutrifft.}
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
Die zugehörige Potenzmenge dieser Menge bildet, wie im Kapitel~\nameref{sec:hasse} beschrieben, alle Knoten des erzeugten Diagramms, einschließlich des größten Begriffs, der hier der leeren Menge entspricht. 
...
% Kontext durch Verband ersetzt
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
%% Nachfolgender Satz neu, bitte prüfen
%% Ebenso wurde in einem späteren Entwicklungsstadium die Darstellung des größten Begriffs abgeschafft, ...
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
Ebenso wurde in einem späteren Entwicklungsstadium auf die Darstellung des größten Begriffs verzichtet, da dessen Merkmalmenge jeweils leer ist und somit keinen Mehrwert zur Exploration des Verbands bietet. 

Die erzeugten Darstellungen sind interaktiv gestaltet, indem Knoten des Diagramms klickbar sind (Abbildungen im Anhang unter \emph{~\nameref{sec:visualization_01}} auf Seite~\pageref{sec:visualization_01}). 
...
Im Hinblick auf die praktische Verwendung ist der erstgenannte Ansatz mit dem manuellen Synonymwörterbuch daher nicht geeignet. 
Fast alles, was die Formale Begriffsanalyse später überhaupt leistet, ist im Vorfeld schon manuell erledigt worden, indem über die Synonymlisten bereits eine Ordnung geschaffen wurde.
...
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
%% TODO: nächster Satz. 1. Welche Literatur? Hinweis vielleicht weglassen. 2. Tempus zwischen 'ausgesucht wurden, ' und 'möchte.'.
% 'möchte' -> 'wollte'
% Bzg. Literatur: ich gehe ja im darauffolgenden Satz auf Janssen ein. Denke das reicht.
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
...
% Punkt am Ende vor der Klammer entfernt
...
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
% Hier eine Ergänzung in Klammern hinzugefügt
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
...
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
%% Referenz auf den Wikipedia-Abschnitt hinzugefügt
%% Danke. \nameref daraus gemacht.
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
Basierend auf den als relevant erachteten Kategorien der englischen Wikipedia, wurden alle Artikel, denen ein gegebenes Kategorie-Tag zugewiesen war, geholt (siehe auch\emph{~\nameref{subsec:wiki_cats}} auf Seite~\pageref{subsec:wiki_cats}).
...
\begin{lstlisting}[caption={DOT-Syntax für das Ordnungsdiagramm des Begriffsverbandes für das Synset \emph{Macula}. Vereinfachte Darstellung. Die URL in den Knoten werden in der resultierenden SVG-Darstellung auf der Seite in Hyperlinks umgewandelt.}]
strict digraph G {
    0 [ label=190, 
...
&   \multicolumn{8}{c}{1}   &   \multicolumn{8}{c}{2}   &   \multicolumn{6}{c}{3}   &   \multicolumn{5}{c}{4}\\
