...
\begin{itemize}
\item Lageinformation wie in der Anatomie, wie zum Beispiel `vorne, hinter, über, in, teilt, beinhaltet' und
\item Bezeichnungen aus dem näheren Umfeld von Krankheiten. wie zum Beispiel `fiebrig, geschwollen, eiternd'
\end{itemize}
...
Weiterhin adjektivische Elemente, die den Begriff einordnen in den Rahmen der durch die eine hierarchische Relation implizierende Wendungen `ist ein', `ist der/die/das' und ähnliche gesteckt wird. 
...
Der Grund dafür ist, dass ein ganzer Bereich unterhalb der Kategorie `Eye' berühmte Augenärzte und Wissenschaftler der Augenheilkunde beinhaltet {[}Grafik{]}.
Ein anderer Bereich hingegen gehört zu Unterkategorien der Kategorie `blindness' und dort tauchen sämtliche Personenartikel in der Wikipedia auf, deren Gegenstand auch ein blinder Mensch ist (diese umfassen unter anderem Kategorien wie `blind academics', `fictional blind characters' oder `sportspeople with a vision impairment'\footnote{In dieser Katgorie finden sich tatsächlich auch Leute, die nicht blind sind. Das sind zum Beispiel Personen, die sich für die Belange von Blinden verdient gemacht haben.}. 
