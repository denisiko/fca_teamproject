...
Für Kontexte mit geringer Komplexität ist der Aufbau eines hinreichenden Merkmalsraums einfach, wie beispielsweise bei der Untersuchung der Menge der natürlichen Zahlen daraufhin, ob sie gerade, ungerade und/oder eine Primzahl sind (die entsprechenden Merkmale sind \emph{gerade}, \emph{ungerade} und \emph{Primzahl}).
...
Aus der deutschen Wikipedia konnten 433 Artikel für die weitere Verwendung extrahiert werden und aus der englischen Wikipedia 731 Artikel, die Mitglieder der Kategorie selbst oder einer Unterkategorie von \textit{Augenheilkunde} oder \emph{Ophthalmology} waren. 
...
Ein allgemeines Beispiel dafür ist die Kategorie \enquote{Articles needing attention}, die selbst wiederum mehrere hundert Unterkategorien enthält (unter anderem zu der Zeit \enquote{Medical Articles needing attention}). 
...
Auf diese Weise identifizierte Begriffe, zu denen ein Wikipediaartikel existiert, bilden Merkmale, die dem Synset, das dem untersuchten Wikipediaartikel entspricht, zugewiesen werden. 
Wenn beispielsweise die Wortfolge \textit{Diabetes mellitus} im Abstract des Artikels 'Diabetische Retinopathie' gefunden wurde, dann wird das Merkmal \textit{Diabetes mellitus} dem Synset 'diabetische Retinopathie' zugeordnet.
...
\begin{itemize}
\item Lageinformation ähnlich jenen in der Anatomie, wie zum Beispiel \textit{vorne, hinter, über, in, teilt, beinhaltet} und
...
Der Grund dafür ist, dass ein ganzer Bereich unterhalb der Kategorie \enquote{Eye} berühmte Augenärzte und Wissenschaftler der Augenheilkunde beinhaltet {[}Grafik{]}.
Ein anderer Bereich hingegen gehört zu Unterkategorien der Kategorie \enquote{blindness}\footnote{In dieser Katgorie finden sich tatsächlich auch Leute, die nicht blind sind. Das sind zum Beispiel Personen, die sich um die Belange von Blinden verdient gemacht haben.} und dort tauchen sämtliche Personenartikel in der Wikipedia auf, deren Gegenstand auch ein blinder Mensch ist (diese umfassen unter anderem Kategorien wie \enquote{blind academics}, \enquote{fictional blind characters} oder \enquote{sportspeople with a vision impairment}. 
...
Der Aufbau eines formalen Begriffsverbandes für das Wörterbuch hat für die Nutzer nur dann einen Mehrwert, wenn ein angemessenes Interface zur Betrachtung und Steuerung des Verbands vorliegt. 
...
Unser Script zur Visualisierung eines formalen Kontextes erzeugt mithilfe des Graphenerstellungsprogramms \emph{GraphViz} ein Ordnungsdiagramm ausgehend von einem ausgewählten Synset. 
...
Bei Klick auf einen Knoten öffnet sich ein Menü, das sowohl die Merkmale, als auch die Vorzugsbenennungen der zugehörigen Synsets anzeigt. 
...
%% ANTWORT: Die Zahlen hierzu stehen unter Ergebnisse des Wikipedia-Teils, ein paar Absätze weiter oben
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
...
Dieser Ansatz ist aber noch weiter zu verschärfen, indem man die zu verwendenden Merkmale selbst vorgibt und den Zuweiungsschritt auf das Zuweisen von extrahierten Merkmalen zu vorher festgelegten verschiebt.
...
Bei der automatisierten Extraktion von Merkmalen aus externen Wissensquellen, wie der Wikipedia, entstehen mit hoher Wahrscheinlichkeit zu allgemeine Merkmale wie Tier, Reittier oder Huf.
...
Solcherlei Synsets sollten nach Aspekten unterscheiden bzw. gruppieren, die sehr gehäuft in der Domäne der Augenheilkunde auftreten und in der Betrachtung eines Fachkundigen Relevanz haben (zum Beispiel Krankheit, Organ, Behandlungsmethode, diabetisch, usw.). 
Gründe dafür sind:
\begin{itemize}
...
\item bei Erstellung des Merkmalraums mit intellektuellem Aufwand und mithilfe von Fachwissen hat man deutlich mehr Kontrolle über die Bildung des Begriffsverbandes
\end{itemize}
...
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
%% Mein Vorschlag zum Unterkapitel 'Verschiedenes': raus damit!^^ Haben eh schon mehr als genug
