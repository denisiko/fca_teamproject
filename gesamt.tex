...
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
%% TODO: Der Satz soll doch eigentlich sagen: Die Synsets im Wörterbuch sind nicht gleichmäßig über vorhandenen Sprachen verteilt.
...
Zu diesem Zweck wurde von beiden der Volltext-Dump der Wikipedia Abstracts als XML-formatierte Datei heruntergeladen und innerhalb der \textit{title}-Tags nach Übereinstimmungen mit Begriffen oder Phrasen aus dem Wörterbuch der Augenheilkunde gesucht. 
...
Die Überlegungen zur Erzeugung von Merkmalszuweisungen wurden daher darauf konzentriert, aus den ersten beiden Sätzen des Abstracts eine Zerlegung in Wort-n-Gramme der Längen 2 und 3 zu erstellen, um mit diesen mittels Zeichenkettenvergleich weitere Titel von Wikipedia-Artikeln aus dem Dump zu identifizieren.
...
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
%% Nachfolgende 2 Punkte geändert
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
...
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
%% ANMERKUNG Du meinst im folgenden Satz wohl 'stellte sich als zutreffend  heraus', ohne das 'nicht', oder?
%% Fixed.
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
...
\subsection{Die Systematisierte Nomenklatur der Medizin}
...
Für die meiste Zeit des Projekts waren wir aber beschäftigt eine möglichst große Merkmalsmenge zu finden, ohne auf die Passung mit dem Wörterbuch der Augenheilkunde Rücksicht zu nehmen.
Der Versuch die Begriffe des Wörterbuchs über die Titel der Wikipedia-Artikel zur Passung zu bringen und im Erfolgsfall überhaupt Merkmale zuzuweisen, zeigte sich als deutliche Einschränkung.
...
Wir wissen also eigentlich gar nicht, was die Bedingungen sind, die man im Hinblick auf die Merkmalsmenge braucht, um die Formale Begriffanalyse besser in einem Umfeld wie dem Wörterbuch der Augenheilkunde nutzbar zu machen, weil wir zu diesem Punkt nicht gekommen sind.
Dazu hätten wir für die Evaluation geeignete Teilmengen der Merkmalsmenge benötigt.
Aus unserer Sicht wäre es günstig gewesen diese Tests an den Wikipedia-Artikeln, aus denen die Merkmale ursprünglich stammten, vorzunehmen und das Wörterbuch der Augenheilkunde anfangs außen vor zu lassen, bis die Frage der geeigneten Merkmalsmenge geklärt ist.
...
Anders als Tagclouds und ähnliche \enquote{siehe auch}-Navigationselemente, fußt der gezeigte Begriffsverband auf hierarchisch korrekter Information. 
...
Im Hinblick auf die Projekplanung wäre unsere Wunschvorstellung bezüglich der Aufgabenstellung, jetzt wo wir wissen was wir wissen, folgendermaßen:

\begin{enumerate}
    \item Identifizieren einer geeigneten Quelle zur Merkmalsextraktion (läuft auf Wikipedia hinaus)
    ...
    \item erst am Schluß der Versuch eine Anbindung an das Wörterbuch der Augenheilkunde zu bekommen, gegebenenfalls durch Ergänzen mit in der Wikipedia vorhandenen Einträge und Umbenennen von Einträgen für eine bessere Passung mit den Einträgen aus der Wikipedia.
\end{enumerate}
