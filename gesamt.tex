\documentclass[pagesize,DIV=calc,12pt,final]{scrreprt}
\usepackage{scrhack}
...
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
Basierend auf den Kategorien, wurden alle Artikel, denen ein gegebenes Kategorie-Tag zugewiesen war geholt (genaueres, siehe Abschnitt \emph{Die Kategorie-Tags in der Wikipedia}).
...
\subsection{Erzeugung eines Graphen}

Beispielhaft wird an dieser Stelle das DOT-Format (Auszeichnungssprache für GraphViz) eines automatisiert erzeugten Graphens gezeigt, aus dem die visuelle Repräsentation des Graphen mittels eines Renderers ins SVG-Format geparst wird. Erzeugt wurde der Graph aus dem Begriffsverband des Synsets Macula mit dem Script visualize_new.php. Anhand des Formats erkennt man deutlich Knoten und Kanten, sowie deren Attribute (Labels, Farben,  Verlinkungen, etc.).
...
\begin{lstlisting}
strict digraph G {
	graph [center=true, size=12, overlap=voronoi, splines=true, sep="0.01"];
	node [label="\N"];
	graph [bb="0,0,142,260"];
	0 [label=190, URL="/visualize_new.php?WID=1895&show=1", pos="31,167", width="0.86111", height="0.51389"];
	1 [label=598, URL="/visualize_new.php?WID=1895&show=2", pos="31,241", width="0.86111", height="0.51389"];
	2 [label=46, URL="/visualize_new.php?WID=1895&show=4", style=filled, color="#333344", fontcolor="#ffffff", pos="69,19", width="0.75", height="0.51389"];
	3 [label=103, URL="/visualize_new.php?WID=1895&show=8", pos="111,241", width="0.86111", height="0.51389"];
	4 [label=50, URL="/visualize_new.php?WID=1895&show=9", pos="69,93", width="0.75", height="0.51389"];
	5 [label=79, URL="/visualize_new.php?WID=1895&show=10", pos="107,167", width="0.75", height="0.51389"];
...
function getPowerset($array) {
	...
	$power = array(array());
	foreach ($array as $element) foreach ($power as $combi)
...
\begin{lstlisting}
function is_subset($a1, $a2) {		//Überprüfung, ob $a1 Untermenge von $a2 ist

	foreach ($a1 as $word) if (!in_array($word, $a2)) return false;
	return true;
}

function is_transitive($edge) {		//Überprüfung, ob Kante überflüssig ist

	global $edges;
	foreach ($edges as $ecomp) if ($edge != $ecomp && $edge[0] == $ecomp[0]) {	//wenn Ausgangsknoten beider Kanten gleich ist...
		if (sameRoute($edge, $ecomp)) return true;				//... dann check, ob Alternativpfad existiert
	}
	return false;
}

function sameRoute($ebasic, $ecomp) {		//Überprüfung, ob $ecomp über andere ausgehende Kanten zum Zielknoten von $ebasic führt

	global $edges;
	if ($ebasic[1] == $ecomp[1]) return true;	//true-Bedingung: Zielknoten sind identisch = Kante überflüssig (wg. Alternativroute)
	foreach ($edges as $next) if ($ecomp[1] == $next[0]) {	//wenn ausgehende Kante...
		if (sameRoute($ebasic, $next)) return true;	//... dann rekursiv mit dieser Kante Überprüfung fortsetzen
	}
	return false;
}
\end{lstlisting}
